...
//    const bool replace_dir = true;
    const int16_t generations = 10000;  // Should be 10k+
    // Use '0' for a random seed
    const unsigned int seed = 0;
    // Forest
    // ======
    const int16_t n_pop = 10;
    // Tree
    // ====
    const int16_t maturity = 4;
    // Make sure to balance this with maturity otherwise you wont have enough memory
    const uint16_t gene_activation_length = 3;
    const float core_gene_substitution_chance = 0.5;
    const uint16_t collision_precision = 1000;
    ...
    // Genome
    // ======
    const uint16_t start_genome_size = 5;
    // ATTENTION: THIS HAS TO BE LESS THAN MAX_CHAR - 48!!!
    const uint16_t max_genome_size = 10;
